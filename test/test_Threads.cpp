...
void allocation_task(int task_id)
{
    try
    {
        ...
        for (int i = 0; i < NODES_PER_TASK; ++i)
        {
            ...
            {
                // Write the task ID into the structure members
                // This proves that each returned pointer refers to isolated, valid memory.
                allocated_nodes[i]->nodeId = static_cast<uint32_t>(task_id * 1000 + i);
...
bool run_memory_pool_stress_test()
{
    ...
    if (concurrency == 0)
        concurrency = 4; // Fallback if detection fails
